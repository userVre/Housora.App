...
export const getProject = query({ args:{ projectId:v.string()}, handler: async(ctx,{projectId})=> await ctx.db.get(projectId as any)});
...
export const createRoom = mutation({
  ...
  handler: async(ctx,a)=>{ const ownerId=await owner(ctx); return await ctx.db.insert("housoraRooms",{ownerId,...a, createdAt:Date.now()}); }
});
export const listRooms = query({ args:{ projectId:v.string()}, handler: async(ctx,{projectId})=> await ctx.db.query("housoraRooms").withIndex("by_project",q=>q.eq("projectId",projectId)).collect()});
...
export const upsertStyleLibrary = mutation({
  ...
  handler: async(ctx,a)=>{ const ownerId=await owner(ctx); const existing=await ctx.db.query("housoraStyleLibraries").withIndex("by_project",q=>q.eq("projectId",a.projectId)).first(); if(existing){ await ctx.db.patch(existing._id,{...a, lockedAt: a.locked?Date.now():undefined}); return existing._id;} return await ctx.db.insert("housoraStyleLibraries",{ownerId,...a, lockedAt: a.locked?Date.now():undefined, createdAt:Date.now()}); }
});
export const getStyleLibrary = query({ args:{ projectId:v.string()}, handler: async(ctx,{projectId})=> await ctx.db.query("housoraStyleLibraries").withIndex("by_project",q=>q.eq("projectId",projectId)).first()});
...
export const listMembers = query({ args:{ projectId:v.string()}, handler: async(ctx,{projectId})=> await ctx.db.query("projectMembers").withIndex("by_project",q=>q.eq("projectId",projectId)).collect()});

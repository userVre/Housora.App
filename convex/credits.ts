...
async function createFreeAccount(ctx: any, ownerId: string, now: number) {
  ...
  await ctx.db.insert("creditTransactions", {
    ownerId,
    eventId: `welcome:${ownerId}`,
    type: "welcome_grant",
    description: "Free trial credits",
    subscriptionDelta: 12,
    purchasedDelta: 0,
    createdAt: now,
  });
...
async function refreshSubscription(ctx: any, account: any, now: number) {
  if (
    ...
    now < account.periodEndsAt ||
    (account.accessEndsAt && now >= account.accessEndsAt)
  ) return account;
  ...
  await ctx.db.patch(account._id, {
    subscriptionCredits: account.monthlyAllowance,
    periodStartedAt,
    periodEndsAt,
    updatedAt: now,
  });
  await ctx.db.insert("creditTransactions", {
    ...
    eventId: `period:${account.ownerId}:${periodStartedAt}`,
    type: "subscription_refresh",
    description: "Monthly plan credits refreshed",
    subscriptionDelta: account.monthlyAllowance - account.subscriptionCredits,
    purchasedDelta: 0,
    createdAt: now,
  });
...
async function purchasedBalance(ctx: any, ownerId: string, now: number) {
  const grants = await ctx.db
    ...
    .collect();
  return grants
    .filter((grant: any) => grant.status === "active" && grant.expiresAt > now)
    .reduce((sum: number, grant: any) => sum + grant.remaining, 0);
}
...
export const getMyBalance = query({
  ...
  handler: async (ctx) => {
    ...
    if (!account) {
      return { plan: "free", status: "active", subscription: 12, purchased, total: 12 + purchased };
    }
...
export const refundUsageServer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.purchasedAmount > 0) {
      await ctx.db.insert("creditGrants", {
        ownerId: args.ownerId,
        sourceEventId: args.eventId,
        kind: "usage_refund",
        amount: args.purchasedAmount,
        remaining: args.purchasedAmount,
    ...
    await ctx.db.insert("creditTransactions", {
      ...
      subscriptionDelta: args.subscriptionAmount,
      purchasedDelta: args.purchasedAmount,
      createdAt: now,
    });
  },
});
...
export const refundUsageEventServer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const refundEventId = `refund:${args.usageEventId}`;
    const duplicate = await ctx.db.query("creditTransactions").withIndex("by_event", (q) => q.eq("eventId", refundEventId)).unique();
    if (duplicate) return;
    const now = Date.now();
    let account = await getAccount(ctx, args.ownerId);
    if (!account) account = await createFreeAccount(ctx, args.ownerId, now);
    const subscriptionAmount = Math.abs(usage.subscriptionDelta);
    const purchasedAmount = Math.abs(usage.purchasedDelta);
    await ctx.db.patch(account._id, { subscriptionCredits: account.subscriptionCredits + subscriptionAmount, updatedAt: now });
    if (purchasedAmount > 0) {
      await ctx.db.insert("creditGrants", { ownerId: args.ownerId, sourceEventId: refundEventId, kind: "usage_refund", amount: purchasedAmount, remaining: purchasedAmount, expiresAt: now + 365 * DAY, status: "active", createdAt: now });
    }
...
export const fulfillWhopServer = mutation({
  args: {
    ...
    offerKey: v.optional(v.string()),
    paymentId: v.optional(v.string()),
    membershipId: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    requireServerKey(args.serverKey);
    const seen = await ctx.db.query("webhookEvents").withIndex("by_provider_and_eventId", (q) => q.eq("provider", "whop").eq("eventId", args.eventId)).unique();
    ...
    if (!ownerId && args.paymentId) {
      ...
      if (ownerId) account = await getAccount(ctx, ownerId);
    }
    await ctx.db.insert("webhookEvents", { provider: "whop", eventId: args.eventId, eventType: args.eventType, ownerId, receivedAt: now });

    if (!ownerId) return { duplicate: false, ignored: true };
    if (!account) account = await createFreeAccount(ctx, ownerId, now);

    if (["refund.created", "dispute.created", "membership.deactivated"].includes(args.eventType)) {
      if (args.paymentId) {
        const grants = await ctx.db.query("creditGrants").withIndex("by_payment", (q) => q.eq("paymentId", args.paymentId)).collect();
        for (const grant of grants) await ctx.db.patch(grant._id, { remaining: 0, status: "reversed" });
      }
      if (args.eventType === "membership.deactivated" || account.lastPaymentId === args.paymentId) {
        await ctx.db.patch(account._id, { status: "inactive", subscriptionCredits: 0, updatedAt: now });
      }
      await ctx.db.insert("creditTransactions", { ownerId, eventId: args.eventId, type: "payment_reversed", description: "Payment, membership, or credit grant reversed", subscriptionDelta: 0, purchasedDelta: 0, createdAt: now });
    ...
    const subscription: Record<string, { allowance: number; accessDays: number }> = {
      ...
      studio_monthly: { allowance: 400, accessDays: 35 },
      studio_yearly: { allowance: 400, accessDays: 370 },
    };
    ...
    if (plan) {
      await ctx.db.patch(account._id, {
        plan: args.offerKey,
        status: "active",
        subscriptionCredits: plan.allowance,
        monthlyAllowance: plan.allowance,
        periodStartedAt: now,
        periodEndsAt: now + MONTH,
        accessEndsAt: now + plan.accessDays * DAY,
        whopMembershipId: args.membershipId,
        lastPaymentId: args.paymentId,
        updatedAt: now,
      });
      await ctx.db.insert("creditTransactions", { ownerId, eventId: args.eventId, type: "subscription_grant", description: `${args.offerKey} plan credits`, subscriptionDelta: plan.allowance - account.subscriptionCredits, purchasedDelta: 0, createdAt: now });
      return { duplicate: false };
    }
    const packAmount = packs[args.offerKey];
    if (packAmount) {
      await ctx.db.insert("creditGrants", { ownerId, sourceEventId: args.eventId, paymentId: args.paymentId, kind: args.offerKey, amount: packAmount, remaining: packAmount, expiresAt: now + 365 * DAY, status: "active", createdAt: now });
      await ctx.db.insert("creditTransactions", { ownerId, eventId: args.eventId, type: "credit_pack", description: `${packAmount} purchased credits`, subscriptionDelta: 0, purchasedDelta: packAmount, createdAt: now });
    }
    return { duplicate: false };
  },
});

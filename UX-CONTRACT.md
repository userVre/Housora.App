...
The project image inspector uses two local tabs: Design room and Edit objects. Design room contains mode, room/building/garden type, style, prompt, and optional mode-specific details. Detected objects is empty before upload. On entering Edit with a new image, offer a credit confirmation before sending pixels to SAM. Never render hard-coded/demo detections. Show only returned masks with real crops, confidence and normalized bounds; a scan may miss objects. Switching tabs retains results; replacing the image resets them.
...
- Album header owns the 3D studio entry in a wide responsive dialog. A detected object's 3D action passes its actual masked crop to the same studio; upload/replacement remains available. Opening the studio itself costs nothing. AR is an outcome inside a completed model preview, not a permanent editor tab; unsupported devices retain the rotatable 3D preview.

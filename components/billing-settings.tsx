...
export function SettingsPage({ onPricing }: { onPricing: () => void }) {
  ...
  return <div className="settings-page">
    ...
    <div className="settings-layout"><nav aria-label="Settings sections">{sections.map(section => <button className={tab === section ? "active" : ""} onClick={() => setTab(section)} key={section}>{section}</button>)}</nav><section className="settings-panel">
      ...
      {tab === "Team" ? <SettingsSection icon={<UserCircle />} title="Team & sharing" description="Collaboration is being prepared for a future release."><div className="team-integrations"><span><small>Design services</small><b>Availability is checked when you use each tool.</b></span></div><Field label="Invite by email"><input placeholder="teammate@studio.com" disabled aria-describedby="team-invite-help" /></Field><button className="primary-action" disabled>Invites coming soon</button><div id="team-invite-help" className="settings-hint">Team invitations are not available yet. Project sharing will appear here after email delivery and permission management are fully connected.</div></SettingsSection> : null}

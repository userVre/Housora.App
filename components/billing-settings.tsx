...
export function SettingsPage({ onPricing }: { onPricing: () => void }) {
  ...
  const persist = async () => {
    ...
    setNotice("");
    try {
      await save({ ...form, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || form.timezone });
  ...
  return <div className="settings-page">
    ...
    <div className="settings-layout"><nav aria-label="Settings sections">{sections.map(section => <button className={tab === section ? "active" : ""} onClick={() => setTab(section)} key={section}>{section}</button>)}</nav><section className="settings-panel">
      {tab === "Profile" ? <SettingsSection icon={<UserCircle />} title="Profile" description="Your identity is securely managed by Clerk."><ReadOnly label="Name" value={user?.fullName || "Not set"}/><div className="settings-hint">Edit name in Manage account and security.</div><ReadOnly label="Email" value={user?.primaryEmailAddress?.emailAddress || "Not set"}/><button onClick={() => openUserProfile()}>Manage account and security</button></SettingsSection> : null}
      {tab === "Workspace" ? <SettingsSection icon={<GearSix />} title="Workspace" description="Regional details used across projects. Timezone is auto-detected."><Field label="Studio name"><input value={form.studioName} onChange={e => set("studioName", e.target.value)} placeholder="e.g. Ismail Studio" /></Field><div className="settings-pair"><Select label="Language" value={form.language} values={["English", "French", "Arabic"]} onChange={v => set("language", v)}/><Select label="Measurements" value={form.measurements} values={["Metric", "Imperial"]} onChange={v => set("measurements", v)}/></div><div className="settings-pair"><Select label="Currency" value={form.currency} values={["USD", "EUR", "GBP", "MAD"]} onChange={v => set("currency", v)}/><Field label="Timezone"><input value={Intl.DateTimeFormat().resolvedOptions().timeZone} disabled title="Auto-detected from your browser" /></Field></div><div className="settings-hint">Timezone auto-detected: {Intl.DateTimeFormat().resolvedOptions().timeZone}</div></SettingsSection> : null}
      ...
      {tab === "Team" ? <SettingsSection icon={<UserCircle />} title="Team & sharing" description="Invite collaborators and see integrations status."><div className="team-integrations"><span><small>Integrations (server-only, no keys shown)</small><b>CubiCasa ✓ Connected</b><b>Sketchfab ✓ Connected</b><b>Tripo ✓ Connected</b><b>SAM 3.1 ✓ Connected</b></span></div><Field label="Invite by email"><input placeholder="teammate@studio.com" id="team-invite-email" /></Field><button className="primary-action" onClick={() => { const el=document.getElementById("team-invite-email") as HTMLInputElement|null; if(el?.value) { setNotice(`Invite sent to ${el.value} (demo)`); el.value=""; }}}>Send invite</button><div className="settings-hint">Roles: owner (you) • designer (edit) • client_viewer (comment/approve only). Managed via projectMembers.</div></SettingsSection> : null}
      {tab === "Notifications" ? <SettingsSection icon={<Sparkle />} title="Notifications" description="Choose which useful updates Housora may send."><Toggle label="Generation updates" hint="Tell me when a longer render finishes." checked={form.generationNotifications} onChange={v => set("generationNotifications", v)}/><Toggle label="Low-credit alerts" hint="Warn me before my balance interrupts a project." checked={form.creditNotifications} onChange={v => set("creditNotifications", v)}/><Toggle label="Collaboration updates" hint="Invites, comments and approvals." checked={form.collaborationNotifications} onChange={v => set("collaborationNotifications", v)}/><div className="settings-hint">Marketing emails are off by default to keep focus.</div></SettingsSection> : null}
      ...
      {tab === "Privacy & data" ? <SettingsSection icon={<ShieldCheck />} title="Privacy and data" description="Optional analytics remain off unless you choose to enable them."><Toggle label="Product analytics" hint="Share interaction events that help improve Housora. Prompts and uploaded images are excluded." checked={form.analyticsConsent} onChange={v => set("analyticsConsent", v)}/><div className="settings-legal"><Link href="/privacy">Read Privacy Policy</Link><Link href="/terms">Read Terms of Service</Link></div><button onClick={() => openUserProfile()}>Manage security or delete account</button><div className="settings-hint">Session replay removed for launch to keep privacy simple.</div></SettingsSection> : null}

...
export function PricingPage() {
  ...
  return <div className="commerce-page">
    ...
    <section className="credit-guide"><div><span className="eyebrow">What credits cover</span><h2>One balance, clear costs.</h2></div><div className="credit-costs"><span><b>1</b> Detect objects</span><span><b>4</b> Generate or edit an image</span><span><b>5</b> Detect + edit one object</span><span><b>12</b> Create a 3D model</span><span><b>0</b> View a model in AR</span></div></section>
    <section className="topup-section"><div><span className="eyebrow">Extra credits</span><h2>Top up without changing plans.</h2><p>Purchased credits are used after plan credits and expire after 12 months.</p></div><div className="pack-grid">{packs.map(pack => <button key={pack.key} className={pack.best ? "best" : ""} disabled={Boolean(pending)} onClick={() => checkout(pack.key)}><span><b>{pack.credits} credits</b>{pack.best ? <small>Best value</small> : null}</span><strong>{pack.price}</strong></button>)}</div></section>

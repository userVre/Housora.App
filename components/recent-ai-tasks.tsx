...
export function RecentAiTasks({ onOpen }: { onOpen: (image: string, objects?: DetectedObject[], mode?: string, projectId?: string, roomId?: string) => void }) {
    ...
    <ul>{jobs.map(job => <li key={job._id}>
      ...
      {job.status === "success" && job.result?.payloadUrl ? <button disabled={opening !== null} onClick={async () => {
        ...
        try {
          ...
          onOpen(image, body.result.objects, job.mode, (job as any).projectId, (job as any).roomId);

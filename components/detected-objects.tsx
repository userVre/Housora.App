...
export function DetectedObjects({ hasImage, mode, image, onUpload, onImageChange, active = true, onSelect, onCreate3d, initialObjects }: {
  ...
  if (!hasImage) return <div className="objects-empty"><MagnifyingGlass /><h3>Start with your photo</h3><p>Find objects in your photo, then choose what to edit.</p><button onClick={onUpload}><UploadSimple /> Upload a photo</button></div>;
  return <div className="real-objects-panel">
    ...
    {status === "detecting" ? <div className="objects-empty" role="status"><span className="spinner" /><h3>Finding objects in your photo…</h3><p>The first scan can take a few minutes while the model starts. Keep this workspace open.</p></div> : objects.length === 0 ?
      <div className="objects-empty"><MagnifyingGlass /><h3>{status === "error" && isServiceUnavailable ? "Detection offline" : status === "ready" ? "No objects found" : "Detect objects to start editing"}</h3><p>{status === "error" && isServiceUnavailable ? "Object detection will be available soon. Your work is safe — generate or save in the meantime." : status === "ready" ? "Nothing was charged. Try a clearer photo with visible furniture or surfaces." : "Scan this photo for furniture and surfaces. Detections will appear here."}</p><button disabled={busy || isServiceUnavailable} onClick={() => setConfirmation("detect")}>Auto-detect · {AI_COSTS.detection} credit</button>{isServiceUnavailable ? <small className="service-hint">No credits charged while offline</small> : null}</div> :
      <div className="real-object-list" aria-label="Objects detected in this photo">{objects.map((object, index) => <button key={object.id} disabled={busy} aria-pressed={selected?.id === object.id} onClick={() => { setSelected(object); onSelect?.(object); }}>
        <span className="real-object-thumb"><Image src={object.thumbnail} alt="" fill sizes="56px" unoptimized /></span><span><b>{object.label}</b><small>Object {index + 1}</small></span><ArrowRight />
      </button>)}</div>}
